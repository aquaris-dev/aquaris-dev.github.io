---
layout: post
title:  BOISTEROUS BLOG (Device Status) &ndash; October, 2018
category: blog
...
Last month, we started working on the zangya and zangyapro devices. To date, it was our fastest port yet. We officially have LineageOS 15.1 running, with full hardware support[^1], that we have fully tested. Next week, test builds will be available for advanced users on our Slack. 
...
- Removed Double Tap 2 Wake vibration, as it caused random vibrations during a call.[^3]
...
[^1]: Even if it’s enabled, LineageOS does a secondary check, based on your SIM Card, on whether call recording is legal in your country. Even if it is illegal or dubious, if there are precedents that prove that it might be used in some situations, you can submit changes to the LineageOS gerrit, in order to mark your country as one that can pass the secondary check. 
[^2]: This change was merged, because some users’ phones would rattle when they vibrated.
[^3]: This was decided after users voted through our Slack.

**Known bugs:**
- OGG files do not play, if they are above a certain length.
- Some selfie sticks, that use the 3.5mm audio jack to operate, do not work.[^1]
- The quality of voice messages, recorded and sent via Facebook Messenger, is low.[^1]
- The proximity sensor does not properly work for direct calls.[^1]
- Some users have reported higher battery usage, compared to stock.[^2]
- Some users experience WLAN problems when phone switches from one router to another.-  we suspect it’s an issue with the Qualcomm Prima firmware. Contact us if you know more.[^3] 
- NFC Payments aren’t available, due to the unlocked bootloader. Because of *SafetyNet*, this cannot/will not be fixed within LineageOS.

[^1]: These bugs are very hard or even outright impossible for us to fix. These are problems that are situated within the closed-source parts of BQ’s stock low-level firmware or the Messenger app. Because they’re closed-source, we have no way of fixing these problems on our own without a huge amount of reverse engineering work. We are waiting for fixes by these vendors. 
[^2]: Because of the more sophisticated coding that has been created for the bardock and bardockpro LineageOS builds, the devices will use, on average, around 5-7% more battery per day. Anything above this can be viewed as a bug or a problem with your installation. **Please be aware, that modifications like Xposed and Magisk are not supported by us. If you report a bug, make sure it appears on a clean installation of LineageOS, that has not been modified in any way.**
[^3]: This is a known stock firmware issue, which seems to happen only with specific routers used in Germany.
...
--------------------------------------------------
[Numerous changes](https://review.lineageos.org/#/q/project:LineageOS/android_device_bq_msm8916-common) were merged today from guest contributor [Aymen](https://review.lineageos.org/#/q/owner:aymenbousselham%2540gmail.com+status:open), in order to successfully build LineageOS 15.1. 
...
- [Proximity sensor](https://jira.lineageos.org/browse/REGRESSION-776) does not work during calls. (paella)
